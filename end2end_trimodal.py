...
from models.trimodals import E2E_Hierarchical_MCTN_Model
...
# data load
is_cycled = configs['translation1']['is_cycled']
feats_dict = \
...
features = args.feature  # e.g. ['f', 'a', 't']
assert len(features) == 3, 'Wrong number of features'
end2end_model = E2E_Hierarchical_MCTN_Model(configs,
                                            features,
                                            feats_dict)

print("PREP FOR TRAINING...")
filename = '_'.join(args.feature) + "_attention_seq2seq_" + \
           str("bi_directional" if configs['translation1']['is_bidirectional']
               else '') + \
           "_bimodal.h5"
